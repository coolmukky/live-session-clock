import type { EngineSnapshot, RunState, SectionTiming, SessionData } from './types';

const MIN_MS = 60_000;

/**
 * Pure derivation of the live session state for a given instant. Extracted from
 * the React hook so it can be unit-tested without a renderer.
 *
 ...
 * "now", so the schedule shifts to stay honest. When idle the agenda is
 * previewed as if it started now.
 */
